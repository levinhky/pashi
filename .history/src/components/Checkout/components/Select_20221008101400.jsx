...
const Select = ({ control, setValue, name, data, selectLabel, ...props }) => {
  ...
  return (
    <div className="relative" ref={nodeRef}>
      <div
        className="flex items-center justify-between px-3 bg-white border border-gray-100 rounded-lg cursor-pointer"
        onClick={() => setShow(!show)}
      >
        <span>{label}</span>
        <span className="w-[50px] place-content-center bg-center icon">
      ...
      <div
        className={`absolute top-full left-0 w-full rounded-lg bg-white focus:shadow-[0_0_0_2px_#338dbc] focus:outline-none focus:ring-sky-500 delay-300 shadow-md transition-all ${
